...
`LINE_MONTHLY_PUSH_LIMIT` は契約している LINE プランの通数に対する安全弁です（既定180）。
**この上限は全店舗の合計** で管理されます（LINE 公式アカウントは全店舗で1つを共用する構成のため）。

規模に対して初期値のままだと、月の途中で `SKIPPED_QUOTA` として送信が止まります。
在籍キャストが増えたら次の値に更新してください。
...
通数は `npm run docs:simulate` で再算出できます。
