...
上から順に実施すれば完了します。所要時間の目安は
**システムの構築が約90分**、これに加えて**キャストの初期設定が1人あたり2分**です
（4店舗・キャスト100名なら合計およそ5時間。話し方の設定は店長が分担できます）。
...
| `LINE_MONTHLY_PUSH_LIMIT` | 在籍規模に応じた値（手順11の表を参照。無料プランなら `180`） |
...
登録すると、キャストのトーク画面下部に
「投稿したよ」「文面をつくる」「今週の状況」のメニューが表示されます。
...
`LINE_MONTHLY_PUSH_LIMIT` の初期値は **180通** で、1店舗・少人数を想定した安全弁です。
**この上限は全店舗の合計** で管理されるため、規模が大きい場合は必ず引き上げてください。
変更しないと月の途中で送信が自動停止します。

| 在籍キャスト | 最大送信数 | 必要な LINE プラン | 設定する値 |
| ---: | ---: | --- | ---: |
| 5名 | 152通 | フリー（200通） | 180 |
| 30名 | 909通 | ライト（5,000通） | 4,800 |
| 100名 | 3,030通 | ライト（5,000通） | 4,800 |
| 165名超 | 5,000通超 | スタンダード（30,000通） | 契約通数 − 200 |

「最大送信数」は全員が1か月まったく更新しなかった場合の値です。
`npm run docs:simulate` で再算出できます。
...
1人あたり1〜2分です（100名なら約3時間）。**店長権限でも入力できる**ため、店舗ごとに分担してください。
...
### 12-3. 生成される文面を実際に確認する（導入前に必須）

**この確認をせずに引き渡さないでください。**
自動テストはモックサーバー相手のため、実際の文面の質は検証できていません。

```bash
# .env に ANTHROPIC_API_KEY を設定してから
npm run draft:probe
```

4パターン × 2回の生成結果と、文面の重複度・一人称の遵守率・実測費用が表示されます
（1回の実行で約4円）。**出力された文面を必ず人が読み**、お店として出せる文章か確認してください。

判断の目安:

- 別キャスト間の重複度が **30%を超える** → 「話し方の設定」をもっと具体的に書く
- 一人称の遵守率が **80%未満** → 一人称の指定を見直す
- NG表現が頻発する → 店舗の方針（`お店としての文面の方針`）に明記する
...
2. **LINE の無料枠は月200通**。キャスト30名以上ならライトプラン（月5,000円・5,000通）が必要。
   **LINE公式アカウントは全店舗で1つを共用**し、送信上限も全店舗の合計で管理される
