...
**単体テストとE2Eはモックサーバー相手です。**
実際にどんな日本語が返るか、本当に文体が散るか、NG表現が混ざる頻度、
1回いくらかは、本物の API を叩かないと分かりません。

```bash
# .env に実際の ANTHROPIC_API_KEY を設定してから
npm run draft:probe                       # 4パターン × 2回 = 8回生成（約4円）
npm run draft:probe -- --runs 5           # 回数を増やす
...
| 項目 | 見るべき点 |
| --- | --- |
| 生成された文面 | **人が読んで、お店として出せる文章か判断する**（自動判定できない部分） |
| 文面の重複度 | 別キャスト間の平均が **30%を超えたら要対策**（似た文面が横並びになる） |
| 一人称の遵守率 | 「話し方の設定」が実際に効いているか |
| 指定文字数±20%内 | 長さの指定が守られているか |
| NG表現の検出数 | 頻発するなら指示の見直しが必要 |
| 実測トークン数と費用 | 資料に載せた単価とずれていないか |

プロフィール未入力のキャスト（さくら）を意図的に含めてあります。
設定を埋め忘れた店舗で何が起きるかを、導入前に確認するためです。
