...
| LINE リッチメニュー | 「投稿したよ」「文面をつくる」「今週の状況」を1タップで操作 |
| 文面作成 | キャストが LINE から開いてブログの**下書き**をつくれる（投稿はしない。本人がコピーして貼り付け） |
...
| `ANTHROPIC_API_KEY` | 文面作成利用時 | Claude API のキー |
| `CAST_LINK_SECRET` | 文面作成利用時 | キャスト用リンクの署名鍵（32文字以上・専用の値） |
| `ANTHROPIC_MODEL` | 任意 | 使用モデル（既定 `claude-haiku-4-5-20251001`） |
...
「何を書けばいいか分からない」が更新の止まる主因になるため、
キャストが LINE から下書きをつくれる導線を用意しています。

**この機能も外部サイトへの投稿は一切行いません。**
つくるのは下書きだけで、投稿はキャスト本人が内容を確認したうえで手作業で行います。

### 使い方の流れ

1. キャストがリッチメニューの「文面をつくる」をタップ
2. LINE が **60分間有効の署名付きリンク**を返す（Push ではなく応答なので送信数を消費しません）
3. リンク先でテーマとキーワードを入れて生成 → 3案が出る
4. 気に入った案を**ワンタップでコピー**し、自分でブログに貼り付ける
5. 同じ画面の「投稿しました」で更新実績を記録（LINE の自己申告と同じ扱い）

リマインドのメッセージ本文にも同じリンクを同梱しています（別送信ではないため送信数は増えません）。

### 設定

| 場所 | 設定内容 |
| --- | --- |
| `.env` | `ANTHROPIC_API_KEY` / `CAST_LINK_SECRET`（必須）、`ANTHROPIC_MODEL`（任意） |
| 設定画面 → 店舗 → 文面作成 | 有効/無効、月間の作成回数上限、店舗としての方針、書かせたくない表現 |
| キャスト詳細 → 話し方の設定 | 一人称、話し方の特徴、よく書く話題、絵文字の量、個別の NG 表現 |

> **話し方の設定は必ず埋めてください。** 空のままだと店舗内の全員が似た文面になり、
...
- 上限とは別に、**1キャストにつき60秒に3回**の連続実行制限が常に効きます
  （リンクが第三者に渡った場合の悪用と、連打による無駄な課金を防ぐため）
- 当月の作成回数とトークン数は設定画面に表示されます

### 安全側の作り

- 生成後の文面を **NG 表現で機械的に検査**します（指示だけでは混ざることがあるため）
- 連絡先・料金・他店比較・年齢に関する表現は、店舗設定に関わらず**常に禁止**です
- 表記ゆれ（全角/半角、カタカナ/ひらがな、記号挿入）を正規化してから照合します
- 生成しただけでは更新実績になりません（実績の水増しを防ぐため）
